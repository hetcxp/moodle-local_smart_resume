...
namespace local_smartresume\hook;
...
class before_footer {
    ...
    public static function execute(before_footer_html_generation $hook): void {
        ...
        if (!get_config('local_smartresume', 'enable')) {
            return;
        }
        ...
        if ($PAGE->pagelayout === 'course' && strpos($PAGE->url->get_path(), '/course/view.php') !== false) {
            ...
            $strings = [
                'nextactivity' => get_string('nextactivity', 'local_smartresume')
            ];
            ...
            $PAGE->requires->js_call_amd('local_smartresume/main', 'init', [$strings, $first_incomplete_cmid]);
        }
    }
}
